...
const TIPO = {
  "Rosso fermo":    { container: "#FFDAD6", onContainer: "#410002", indicator: "#6D0B0B", label: "🍷" },
...
function FilterChip({ label, active, onClick }) {
  ...
  return (
    <button onClick={onClick} onPointerDown={() => setPressed(true)} onPointerUp={() => setPressed(false)} onPointerLeave={() => setPressed(false)}
      ...
      {t && (<span style={{ display: "inline-flex", alignItems: "center", position: "relative", zIndex: 1 }}><TipoLabel tipo={label} size={13} /></span>)}
...
function WineCard({ wine, expanded, onToggle, onBevi, onElimina, onModifica, bevutoInfo = null, ratings = {}, onRate }) {
  ...
  // ── Scroll automatico all'espansione ──────────────────────────────────────
  // Quando la card si espande, porta il suo header in cima al viewport
  // così il contenuto si apre verso il basso, sempre visibile.
  useEffect(() => {
