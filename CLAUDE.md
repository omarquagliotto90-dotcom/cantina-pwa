...
- **P6** — **in corso (22/09/2026), fase 1 fatta.** Bottiglie come righe: `wines` resta anagrafica, `bevuti` diventa `bottiglie WHERE stato='bevuta'`. Avviato su richiesta di Omar del campo **tipo bottiglia**, che il punto 4 del registro aveva già assegnato qui: su `wines` non ci starebbe, perché l'indice UNIQUE normalizzato è (produttore, vino, annata) e un Magnum finirebbe in upsert sulla riga dello Standard. Essendo il passo meno reversibile, procede in quattro fasi con l'app sempre funzionante (expand/contract):
  - **fase 1 — fatta.** Backup completo in `supabase/backup/20260922_pre_p6.json` (127 vini, 50 bevute, 84 produttori, 125 immagini, 29 siti). Tabella `bottiglie` creata e popolata: 167 righe = 50 da `bevuti` + 117 generate da `wines.bottiglie` con `generate_series`. **Nessuna riga, colonna o RPC esistente è stata toccata**, e niente la legge ancora: si annulla con `drop table`. Verificato dopo la migrazione: somma dei voti 198,7 identica, costo della giacenza 1.578,00 € identico, 1 sola riga senza `wine_id` (la bevuta orfana attesa, il Raina Grechetto del vino 76). `wine_id` è nullable con ON DELETE SET NULL come `bevuti`; lo snapshot produttore/vino/annata/tipologia è valorizzato **solo sulle righe bevute**, perché le giacenze si leggono via join e da P1b `wines` non si cancella più fisicamente
  - **fase 2 — da fare.** Le 9 RPC riscritte per scrivere su `bottiglie` **e** sul vecchio modello, così un rollback resta possibile
  - **fase 3 — da fare.** Il client legge da `bottiglie`. Qui entrano il campo formato nei form e il badge in lista
  - **fase 4 — da fare.** Drop di `wines.bottiglie` e di `bevuti`
  Assorbe i 4 difetti di P4, che decade. Sblocca lo step 9 "cosa bere"
...
- **Il 403 dell'origine, non lo scontorno (21/09/2026).** Sintomo identico a uno scontorno fallito — riquadro bianco intatto attorno alla bottiglia — ma causa opposta: **i byte non arrivano mai**. Il proxy si dichiarava bot (`Mozilla/5.0 (compatible; LaMiaCantina/1.0)`, nessun Referer) e molti siti rispondono 403 a chi non sembra un browser. Misurato interrogando `/api/immagine` in produzione con `web_fetch_vercel_url`: su 6 host fra i piu' numerosi, **3 rifiutano** (vignetiradica.it 5 foto, triplea.it 4, rossopastrengo.com 1) e 3 vanno. In piu' 3 URL sono salvate con schema `http` e `indirizzoAmmesso()` le rifiutava in partenza (ora vengono promosse a https). **Fix:** header da browser vero (UA, `Referer` = origine, `Accept`, `Accept-Language`). E' aggirare una protezione anti-hotlink, deciso da Omar il 21/09/2026: l'attenuante e' che l'app gia' hotlinka quella stessa foto nell'`<img>`, quindi il proxy non prende niente che non si stia gia' prendendo. **Trucco per rimisurare senza scaricare megabyte in contesto:** chiedere al proxy un file inesistente su quell'host — `403` = ancora bloccato, `404` = header accettati, e la risposta e' un JSON di 40 byte in entrambi i casi
- **I fallimenti muti costano un'indagine (21/09/2026).** Tutte le strade che fanno fallire il rifilo — 403, timeout, angoli non concordi, canvas contaminato — finivano nello stesso `catch` e producevano **lo stesso identico risultato sullo schermo**. Guardando uno screenshot era impossibile dire quale fosse: per il caso Malibran e' servito interrogare il proxy a mano. Da qui `motiviRifilo` in `App.jsx`, una mappa in memoria per sessione leggibile da `window.motiviRifilo`; la richiesta in piu' che traduce l'errore parte **solo dopo** che qualcosa e' gia' fallito, una volta per URL. Regola generale per questo progetto: dove piu' cause producono lo stesso pixel, il motivo va registrato, altrimenti ogni diagnosi riparte da zero
- **Le tasche di fondo murate dall'ombra (21/09/2026).** Seguito del punto sopra, trovato sulla foto di Pistis Sophia. Il flood fill avanza sui contigui, quindi un'**ombra** attorno alla bottiglia gli fa da muro: il fondo chiuso dietro l'ombra non viene mai raggiunto e resta come un blocco bianco appiccicato al soggetto. Misurato su tela sintetica: **928 pixel su 928** della tasca restavano opachi. **Allargare la tolleranza del fill non e' la soluzione**: su questa bottiglia l'etichetta e' bianca e gliela mangerebbe. Tasca ed etichetta sono lo stesso bianco — la differenza non e' nel colore ma in **chi le circonda**: una tasca confina quasi tutta con fondo gia' tolto, un'etichetta confina col vetro. Da qui `recuperaTasche()`, seconda passata che raccoglie le componenti connesse di quasi-fondo ancora opache e ne toglie solo quelle cinte per almeno il **70%** da pixel gia' marcati. Terza guardia della serie, e come le altre due nata da un test. La soglia del 70% e' anche cio' che salva una lumeggiatura sul fianco del vetro, che confina col soggetto per meta': c'e' un test apposta, se la soglia si alza cade
- **`mix-blend-mode` muore dentro uno stacking context (21/09/2026). APERTO.** La foto di bottiglia ha `mixBlendMode: "multiply"`, che renderebbe invisibile qualunque bianco residuo. Nell'hero del dettaglio non funziona: il contenuto sta in `sopraFoto = { position: relative, zIndex: 2 }`, e un elemento posizionato con `z-index` **crea uno stacking context**, quindi il blend non raggiunge piu' la foto di regione dietro. Misurato in Chromium sulla stessa pagina, unica differenza il `z-index`: senza → `32,96,192` (il bianco sparisce), con `z-index: 2` → `255,255,255` (bianco pieno). Prima delle foto di regione il fondo era piatto e nessuno se ne accorgeva: **la rete di sicurezza c'era e si e' rotta in silenzio**. Lasciato com'e' per decisione di Omar del 21/09/2026 (togliere l'isolamento farebbe vedere il paesaggio **attraverso** il vetro della bottiglia): l'hero resta opaco e tutto il peso sta sullo scontorno. Da riaprire se un giorno il look "bottiglia translucida sul paesaggio" diventa desiderabile
