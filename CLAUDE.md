...
- **Banda vuota sotto la barra su iPhone (21/09/2026).** **Bug noto di WebKit, non un difetto di layout.** Nelle PWA standalone iOS sottostima `innerHeight`, `clientHeight`, `visualViewport.height` e `100dvh` **esattamente del safe-area superiore**. Misurato su iPhone 14 Pro Max: viewport 873, `screen.height` 932, `safe-area top` 59, e l'app riempiva 873 su 873. Il guscio si ancora correttamente al bordo alto, quindi l'ammanco affiora tutto come banda in fondo. **Con `black-translucent` la webview E' a tutto schermo: sono i numeri a essere corti, non lo spazio** — quindi il CSS ci arriva. Fix: il guscio radice prende `height: calc(100% + env(safe-area-inset-top))`, **solo in standalone** (in Safari e Chrome il viewport è riportato giusto e sommare il safe-area spingerebbe la barra sotto la toolbar del browser). Due strade morte, provate e scartate: `position: fixed; inset: 0` al posto di `100dvh` non cambia niente, e reinstallare l'icona dalla home nemmeno — non è metadato d'installazione congelato. Da tenere d'occhio: `backdrop-filter` su elementi `position: fixed` può promuoverli a un layer di compositing che congela la misura del viewport di quando è stato creato; la barra di navigazione ne ha uno
