...
- **A2a** — in corso. Expand, compatibile con l'App.jsx in produzione, nessun deploy richiesto: 0. backup JSON delle 4 tabelle via curl; 1. elimina l'immagine orfana 76, `bevuti.wine_id` nullable e bevuto orfano a NULL; 2. FK `wine_images` → `wines` ON DELETE CASCADE; 3. FK `bevuti` → `wines` ON DELETE SET NULL + indice su `wine_id`; 4. CHECK su `bottiglie`, `prezzo`, `valore`, `tipologia`, `denominazione`, `rating`; 5. `bevuti.consumed_on date` (backfill da testo italiano, default oggi Europe/Rome, poi NOT NULL); 6. `bevuti.created_at` a `timestamptz`; 7. indice UNIQUE su produttore/vino/annata normalizzati
- **A1** — layer dati nel file: `sb` con AbortController, timeout ed errori tipizzati, distinzione errore/vuoto, estrazione di `useCantinaData()`, funzioni di dominio pure (`valoreBottiglia`, `valoreGiacenza`, normalizzazioni) che unificano le tre formule
