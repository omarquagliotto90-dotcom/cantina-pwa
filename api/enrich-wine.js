// api/enrich-wine.js — Vercel Serverless Function
// Arricchimento dati vino via Claude + web_search tool.
// Richiede ANTHROPIC_API_KEY come variabile d'ambiente su Vercel.

export const config = { maxDuration: 60 };

export default async function handler(req, res) {
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Method not allowed" });
  }

  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    return res.status(500).json({ error: "ANTHROPIC_API_KEY non configurata" });
  }

  const { produttore, vino, annata } = req.body || {};
  if (!produttore || !vino) {
    return res.status(400).json({ error: "produttore e vino sono richiesti" });
  }

  const etichetta = `${produttore} — ${vino}${annata && annata !== "n.d." ? " " + annata : ""}`;

  try {
    const anthropicRes = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": apiKey,
        "anthropic-version": "2023-06-01",
      },
      body: JSON.stringify({
        model: "claude-haiku-4-5-20251001",
        max_tokens: 1000,
        tools: [
          {
            type: "web_search_20250305",
            name: "web_search",
            max_uses: 2,
          },
        ],
        messages: [{
          role: "user",
          content: `Cerca sul web la scheda tecnica e il prezzo medio di mercato di questo vino: ${etichetta}.

Privilegia il sito del produttore per i dati tecnici e i siti di e-commerce/aste vino per il prezzo.

Al termine della ricerca, restituisci come ULTIMA cosa un oggetto JSON (nessun markdown, nessun testo dopo il JSON) con questi campi:
{
  "vitigno": "vitigno/i con eventuali percentuali, oppure ''",
  "macerazione": "descrizione sintetica (es. '15 giorni su bucce'), oppure ''",
  "fermentazione": "descrizione sintetica (es. 'acciaio, lieviti indigeni'), oppure ''",
  "malolattica": "es. 'svolta', 'non svolta', 'parziale', oppure ''",
  "note": "descrizione del vino in 2-3 frasi in italiano, oppure ''",
  "prezzo_stimato": numero in euro per bottiglia 0.75L (es. 18.50), oppure null
}

Regole rigorose:
- NON inventare dati: se un'informazione non emerge dalla ricerca, usa '' (o null per il prezzo).
- I dati devono riferirsi a QUESTO vino specifico (e se possibile a questa annata), non ad altri vini dello stesso produttore.
- prezzo_stimato deve essere un numero, non una stringa né un intervallo.`,
        }],
      }),
    });

    const data = await anthropicRes.json();
    if (!anthropicRes.ok) {
      return res.status(502).json({ error: data.error?.message || "Anthropic error" });
    }

    // Con web_search la risposta contiene blocchi misti (server_tool_use,
    // web_search_tool_result, text): concateniamo solo i blocchi text.
    const text = (data.content || [])
      .filter(c => c.type === "text")
      .map(c => c.text || "")
      .join("")
      .trim();

    // Estrai l'ultimo oggetto JSON presente nel testo
    const start = text.indexOf("{");
    const end = text.lastIndexOf("}");
    if (start === -1 || end === -1 || end < start) {
      return res.status(200).json({ raw: text });
    }

    try {
      const parsed = JSON.parse(text.slice(start, end + 1));
      // Normalizza prezzo: numero o null, mai stringa
      let prezzo = parsed.prezzo_stimato;
      if (typeof prezzo === "string") {
        const n = parseFloat(prezzo.replace(",", "."));
        prezzo = Number.isFinite(n) ? n : null;
      }
      if (typeof prezzo !== "number" || !Number.isFinite(prezzo) || prezzo <= 0) {
        prezzo = null;
      }
      return res.status(200).json({
        vitigno: parsed.vitigno || "",
        macerazione: parsed.macerazione || "",
        fermentazione: parsed.fermentazione || "",
        malolattica: parsed.malolattica || "",
        note: parsed.note || "",
        prezzo_stimato: prezzo,
      });
    } catch {
      return res.status(200).json({ raw: text });
    }

  } catch (err) {
    console.error("enrich-wine error:", err);
    return res.status(500).json({ error: err.message });
  }
}
